...
func (mq *MealQuery) QueryUser() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(meal.Table, meal.FieldID, selector),
...
// First returns the first Meal entity from the query.
// Returns a *NotFoundError when no Meal was found.
...
// FirstID returns the first Meal ID from the query.
// Returns a *NotFoundError when no Meal ID was found.
...
// Only returns a single Meal entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one Meal entity is found.
// Returns a *NotFoundError when no Meal entities are found.
...
// OnlyID is like Only, but returns the only Meal ID in the query.
// Returns a *NotSingularError when more than one Meal ID is found.
...
func (mq *MealQuery) All(ctx context.Context) ([]*Meal, error) {
	...
	qr := querierAll[[]*Meal, *MealQuery]()
	return withInterceptors[[]*Meal](ctx, mq, qr, mq.inters)
}
...
func (mq *MealQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, mq, querierCount[*MealQuery](), mq.inters)
}
...
// Clone returns a duplicate of the MealQuery builder, including all associated steps. It can be
...
func (mq *MealQuery) GroupBy(field string, fields ...string) *MealGroupBy {
	...
	grbuild := &MealGroupBy{build: mq}
	grbuild.flds = &mq.ctx.Fields
	grbuild.label = meal.Label
...
func (mq *MealQuery) Select(fields ...string) *MealSelect {
	...
	sbuild := &MealSelect{MealQuery: mq}
	sbuild.label = meal.Label
...
func (mq *MealQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Meal, error) {
	var (
		nodes       = []*Meal{}
		withFKs     = mq.withFKs
		_spec       = mq.querySpec()
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, meal.ForeignKeys...)
	}
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*Meal).scanValues(nil, columns)
	}
	...
	if query := mq.withUser; query != nil {
		if err := mq.loadUser(ctx, query, nodes, nil,
			func(n *Meal, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MealQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Meal, init func(*Meal), assign func(*Meal, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Meal)
	for i := range nodes {
		if nodes[i].user_meals == nil {
			continue
		}
		fk := *nodes[i].user_meals
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_meals" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MealQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(meal.Table, meal.Columns, sqlgraph.NewFieldSpec(meal.FieldID, field.TypeInt))
	_spec.From = mq.sql
...
func (mq *MealQuery) sqlQuery(ctx context.Context) *sql.Selector {
...
func (mgb *MealGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*MealQuery, *MealGroupBy](ctx, mgb.build, mgb, mgb.build.inters, v)
}

func (mgb *MealGroupBy) sqlScan(ctx context.Context, root *MealQuery, v any) error {
...
func (ms *MealSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*MealQuery, *MealSelect](ctx, ms.MealQuery, ms, ms.inters, v)
}

func (ms *MealSelect) sqlScan(ctx context.Context, root *MealQuery, v any) error {
