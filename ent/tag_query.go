...
func (tq *TagQuery) QueryArticle() *ArticleQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, tag.ArticleTable, tag.ArticleColumn),
		)
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withArticle; query != nil {
		if err := tq.loadArticle(ctx, query, nodes, nil,
			func(n *Tag, e *Article) { n.Edges.Article = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadArticle(ctx context.Context, query *ArticleQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Article)) error {
	...
	for i := range nodes {
		if nodes[i].article_tags == nil {
			continue
		}
		fk := *nodes[i].article_tags
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "article_tags" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
