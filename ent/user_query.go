...
func (uq *UserQuery) QueryMeals() *MealQuery {
	query := (&MealClient{config: uq.config}).Query()
...
// WithMeals tells the query-builder to eager-load the nodes that are connected to
// the "meals" edge. The optional arguments are used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withMeals; query != nil {
...
func (uq *UserQuery) loadMeals(ctx context.Context, query *MealQuery, nodes []*User, init func(*User), assign func(*User, *Meal)) error {
	...
	query.withFKs = true
	query.Where(predicate.Meal(func(s *sql.Selector) {
		s.Where(sql.InValues(user.MealsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_meals
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_meals" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (uq *UserQuery) loadAchievements(ctx context.Context, query *AchievementQuery, nodes []*User, init func(*User), assign func(*User, *Achievement)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_achievements" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
